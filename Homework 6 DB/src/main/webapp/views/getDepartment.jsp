...
<c:if test="${requestScope.page != 1}">
   <span><a href="getDepartment?id=${department.id}&page=1">В начало</a></span>
</c:if>
<c:if test="${page>5}">
   <c:forEach begin="${page-5}" end="${page-1}" var="i">
      <c:if test="${i<=page}">
        <span><a href="getDepartment?id=${department.id}&page=${i}">${i}</a></span>
      </c:if>
   </c:forEach>
</c:if>
...
<c:if test="${requestScope.page != pageCount}">
   <span><a href="getDepartment?id=${department.id}&page=${pageCount}">В конец</a></span>
</c:if>
