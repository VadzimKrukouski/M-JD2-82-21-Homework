...
<c:forEach items="${requestScope.allDepartments}" var="department">
                   <tr>
                       <td><a href="${pageContext.request.contextPath}/department/${department.id}?page=1">${department.name}</a></td>
...
<c:if test="${requestScope.page != 1}">
   <span><a href="${pageContext.request.contextPath}/department/all?page=1">В начало</a></span>
</c:if>
<c:if test="${page>5}">
   <c:forEach begin="${page-5}" end="${page-1}" var="i">
      <c:if test="${i<=page}">
        <span><a href="${pageContext.request.contextPath}/department/all?page=${i}">${i}</a></span>
      </c:if>
   </c:forEach>
</c:if>
...
<c:if test="${requestScope.page != pageCount}">
   <span><a href="${pageContext.request.contextPath}/department/all?page=${pageCount}">В конец</a></span>
</c:if>
